...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(queue.size()>0&&!foundWord2){
			WPTreeNode tn = queue.get(0);
			queue.remove(0);
			List<String> validWords = nw.distanceOne(tn.getWord(), true);
			for(String thisword:validWords){
				if(!(visited.contains(thisword))){
					tn.addChild(thisword);
					visited.add(tn.getWord());
					queue.addLast(new WPTreeNode(thisword, tn));
					if(thisword.equals(word2)){
						foundWord2=true;
